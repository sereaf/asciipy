...
def main():
	...
	parser.add_argument('-au', '--audio', help='DEFAULT:False DESC: With audio', action='store', default=False)
	...
	if file_type == 'video':
		v = AsciiVideo(a.file)
		if output_as == 'save':
			v.ascii_video(output=a.output, audio=a.audio, option=a.option, font=a.font, save_as=a.save_as, scale=a.scale, density_flip=a.density_flip, character_space=a.character_space, chars=a.chars, font_scale=a.font_scale)
